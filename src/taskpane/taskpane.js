...
const openai = new OpenAI({apiKey: '', dangerouslyAllowBrowser: true, version: 'v1'});
    ...
    Office.context.mailbox.getCallbackTokenAsync({isRest: true}, async function(result){
      if (result.status === "succeeded") {
        ...
        const assistant = await openai.beta.assistants.create({
          ...
          instructions: "You are an Outlook QA assistant, who answers questions by making calls to Outlook to retrieve user emails and using that information to successfully answer the users questions. When the user types lets start fresh you clear all messages in the thread.",
          ...
          tools: [{
            ...
            "function": {
              "name": "getAllMessages",
              "description": "Get the user's emails from Outlook based on a search keyword and/or sender email address and/or number of emails to retrieve",
              "parameters": {
                  "type": "object",
                  "properties": {
                      "accessToken": {
                      ...
                      "searchKeyword": {
                          "type": "string",
                          "description": "The keyword to search for in the user's emails and return all emails that match the keyword.",
                      },
                      "from": {
                          "type": "string",
                          "description": "The sender email address to search for in the user's emails and return all emails that match the sender email address.",
                          "unit": {
                            "type": "string",
                            "pattern": "^\\S+@\\S+\\.\\S+$"
                          }
...
function getTextFromHtml(htmlString) {
  ...
  /* Write Code here
  textContent is a string with the body of each email. This is where you need to mask the Private information and return the masked string.
  Either make another function and call it here, or edit this function such that it directly returns the masked string.*/
...
function getAllMessages(params) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (searchKeyword === "" && from !== "None") {
      url = Office.context.mailbox.restUrl + `/v2.0/me/messages?$filter=from/emailAddress/address eq '${from}'&$top=${top}`;
    } else if (searchKeyword !== "" && from === "None") {
      url = Office.context.mailbox.restUrl + `/v2.0/me/messages?$search=${encodeURIComponent(searchKeyword)}&$top=${top}`;
    } else if (searchKeyword !== "" && from !== "None") {
      url = Office.context.mailbox.restUrl + `/v2.0/me/messages?$search=${encodeURIComponent(searchKeyword)}&$filter=from/emailAddress/address eq '${from}'&$top=${top}`;
    }
